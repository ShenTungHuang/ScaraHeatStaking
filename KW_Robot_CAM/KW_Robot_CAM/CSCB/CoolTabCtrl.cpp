...
CCoolTabCtrl::CPageItem* CCoolTabCtrl::AddPage(CWnd *pWnd, LPCTSTR sText, UINT IconID)
{
	...
	CPageItem *pItem = NULL,*pOldItem=NULL;
	
	int itemCount=(int)m_PageList.GetCount();
	int mark=0;
	for (int i=0;i<itemCount;i++)
	{
		pOldItem=(CPageItem*)GetPageItem(i);
		// Add By STH for modify TabCtrl
		CString temp_str;
		temp_str = "STH";
		if( (pOldItem->m_sText==sText) || (pOldItem->m_sText==temp_str) )
		{
			pItem=pOldItem;
			mark=1;
		}
	...
	if (mark==1)
	{
		CWnd* pOldWnd =pItem->m_pWnd;//得到旧对话框句柄
		pOldWnd->DestroyWindow();//删除旧对话框句柄
		pItem->m_pWnd=pWnd;//设置为新对话框句柄
	}
	else
	{
		pItem = new CPageItem();
	...
	if(mark==1)//属性页更改后立马显示出来
	{
		pWnd->ShowWindow(SW_SHOW);

	}
	// Add by STH to modify Simulation tab
	if ( PatternSelect_Mode == true )
	{
		CString temp_str;
		temp_str = "Pattern";
		pItem->m_sText = temp_str;
	}
	return pItem;
}
...
{
	UINT PageCount = (int)m_PageList.GetCount();
	...
	//ClientRect.DeflateRect(m_nBorder+1,m_nBorder+1);//往里收2*****************
	GetWindowRect(rect);
	ScreenToClient(rect);
	...
	ItemMaxWidth =(int) rect.Width()/(int)m_PageList.GetCount();
...
int CCoolTabCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	strcpy_s(logFont.lfFaceName,_T(""));
	logFont.lfHeight = -12;
