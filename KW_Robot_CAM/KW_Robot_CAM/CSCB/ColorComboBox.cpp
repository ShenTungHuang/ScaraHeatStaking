...
//
SColorAndName	CColorComboBox::ms_pColors[ CCB_MAX_COLORS ] =
{
	...
	SColorAndName( RGB( 0x80, 0x80, 0x80 ),	_T("Grey" )),
	...
	SColorAndName( RGB( 0xFF, 0x00, 0xFF ),	_T("Fushcia") ),
...
int CColorComboBox::OnCreate( LPCREATESTRUCT LPCTSTRuct ) 
{
	if( CComboBox::OnCreate( LPCTSTRuct ) == -1 )				// If Create Failed
...
CString		CColorComboBox::GetSelectedColorName( void )
{
	int		iSelectedItem = GetCurSel();					// Get Selected Item
...
int			CColorComboBox::AddColor( LPCTSTR cpName, COLORREF crColor )
{
	int		iItem = -1;
